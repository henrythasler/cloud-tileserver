...
class Projection {
    ...
    getMercatorTileBounds(tile, tileSize = 256) {
        const leftbottom = this.getMercatorFromPixels({ x: tile.x * tileSize, y: (tile.y + 1) * tileSize }, tile.z, tileSize);
        const righttop = this.getMercatorFromPixels({ x: (tile.x + 1) * tileSize, y: tile.y * tileSize }, tile.z, tileSize);
        return { leftbottom, righttop };
    }
    /** Returns bounds of the given tile in WGS84 (https://epsg.io/4326) coordinates */
    getWGS84TileBounds(tile, tileSize = 256) {
        const bounds = this.getMercatorTileBounds(tile, tileSize);
        return {
    ...
    getWGS84TileCenter(tile, tileSize = 256) {
        const bounds = this.getWGS84TileBounds(tile, tileSize);
        return {
